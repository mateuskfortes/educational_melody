...
const appWithAdm = new App({ mysqlStore: false, middleware: [mockSessionAdmin] }).app
...
describe('Lesson', () => {
    ...
    it('Should not create a lesson without content', async () => {
        const res = await request(appWithAdm).post('/lesson/create').send({title: 'My Lesson'})
        expect(res.status).toBe(422)
        expect(res.text).toContain('Conteúdo obrigatório')
    })
    it('Should not create a lesson without title', async () => {
        const res = await request(appWithAdm).post('/lesson/create').send({content: 'My Text'})
        expect(res.status).toBe(422)
