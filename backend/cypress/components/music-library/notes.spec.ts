...
describe('Test notes object', () => {
  it('Should create the note with the given params', () => {
    ...
    expect(note.cleanNote).to.be.equal('C')
    expect(note.octave).to.be.equal(3)
    expect(note.accidental).to.be.equal('sharp')
    expect(note.beatDuration).to.be.equal(defaultWholeBeatDuration * (2 - 1 / Math.pow(2, dots)))
  })
  it('Should create a note with the right beat duration', () => {
    ...
    expect(thirtysecond.beatDuration * 2).to.be.equal(sixteenth.beatDuration)
    expect(sixteenth.beatDuration * 2).to.be.equal(eighth.beatDuration)
    expect(eighth.beatDuration * 2).to.be.equal(quarter.beatDuration)
    expect(quarter.beatDuration * 2).to.be.equal(half.beatDuration)
    expect(half.beatDuration * 2).to.be.equal(whole.beatDuration)
  })
  it('Should create a rest with the right beat duration', () => {
    ...
    expect(thirtysecondRest.beatDuration * 2).to.be.equal(sixteenthRest.beatDuration)
    expect(sixteenthRest.beatDuration * 2).to.be.equal(eighthRest.beatDuration)
    expect(eighthRest.beatDuration * 2).to.be.equal(quarterRest.beatDuration)
    expect(quarterRest.beatDuration * 2).to.be.equal(halfRest.beatDuration)
    expect(halfRest.beatDuration * 2).to.be.equal(wholeRest.beatDuration)
  })
  it('Should set dots with dots limit value if dots value is greater than dots limit', () => {
    ...
    expect(whole.dots).to.be.equal(5) // Whole note has a limit of 5 dots
    expect(half.dots).to.be.equal(4) // Half note has a limit of
    expect(quarter.dots).to.be.equal(3) // Quarter note has a limit of 3 dots
    expect(eighth.dots).to.be.equal(2) // Eighth note has a limit
    expect(sixteenth.dots).to.be.equal(1) // Sixteenth note has a limit of 1 dot
    expect(thirtysecond.dots).to.be.equal(0) // Thirtysecond note has a limit of 0 dots
  })
})