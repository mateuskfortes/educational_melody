...
import { RestPropsTemplate } from "../../../../_types/ComponentsPropsTypes";
...
const Rest = ({ rest, measureIndex, noteIndex }: RestPropsTemplate) => {
  ...
  return (
    <div className="rest_note" style={{ top }}>
      <Image
        src={`images/music-library/${rest.name}.svg`}
        alt={`${ariaLabelStart + rest.getAriaLabel()}, na posição ${
          noteIndex + 1
        } do compasso ${measureIndex + 1}`}
        tabIndex={0}
        className={rest.name}
      />
    </div>
